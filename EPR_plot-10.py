...
def plottaaja_BGR():
    ...
    EPR_amplitude.rename(columns={'index':'Experiment no.', 
                                  0:'EPR signal Amplitude'}, inplace =True) # Rename columns
...
if value==1:
    ...
    EPR_int.plot(ax=axes[1,0],title='1st Integrals', fontsize=8).legend(loc='upper center',bbox_to_anchor=(0.8, 1.05),ncol=2)
    axes[1,0].ticklabel_format(useMathText=True, useOffset=False, style='scientific',scilimits=(0,0))
    
    EPR_int2.plot(ax=axes[1,1],title='2nd Integrals', fontsize=8).legend(loc='upper center',bbox_to_anchor=(0.25, 1.00),ncol=2)
    max_values.plot(ax=axes[2,0],x='Experiment no.',y='DI EPR intensity (A.U.)',kind='scatter')
