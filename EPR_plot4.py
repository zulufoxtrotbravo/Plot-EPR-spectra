...
max_values = pd.DataFrame(EPR_data.max(axis=0)) # Create dataFrame for plotting scatter
max_values.reset_index(inplace=True)            # reset index to plot scatter
max_values.rename(columns={'index':'Experiment no.',
                           0:'EPR intensity (A.U.)'}, inplace=True) # rename columns
max_values_field = pd.DataFrame(EPR_data.idxmax(axis=0))  # Field values corresponding to max_values

max_values.plot(x='Experiment no.',y='EPR intensity (A.U.)',kind='scatter')

#%% Subtract the BGR spectrum from the EPR_data
...
#%% Get maximum values again and plot
max_values = pd.DataFrame(EPR_data_bgr.max(axis=0)) # Create dataFrame for plotting scatter
max_values.reset_index(inplace=True)            # reset index to plot scatter
max_values.rename(columns={'index':'Experiment no.',
                           0:'EPR intensity (A.U.)'}, inplace=True) # rename columns
max_values_field = pd.DataFrame(EPR_data.idxmax(axis=0))  # Field values corresponding to max_values
